...
import static com.yachtmafia.util.Util.getConversionRate;
...
//                        + swapMessage.getToCoinName());
                CoinMarket market = CoinMarketCap.ticker(swapMessage.getFromCoinName()).get();
                double priceUSD = market.getPriceUSD();
                float conversionRate = getConversionRate("USD", swapMessage.getToCoinName());
                String desiredCurrencyValue = String.valueOf(priceUSD * conversionRate);

                addTransactionStatus(swapMessage, StatusLookup.MONEY_WITHDRAWN_FROM_EXCHANGE);


                success = handlerDAO.getBank().payUser(swapMessage.getToCoinName(), desiredCurrencyValue,
                        swapMessage.getUsername());
                if (!success) {
                    logger.error("error when transfering from bank to user: " + swapMessage.toString()
                            + " for amount: " + desiredCurrencyValue);
                    addTransactionStatus(swapMessage, StatusLookup.COULD_NOT_PAY_USER);
                    return false;
                }
                addTransactionStatus(swapMessage, StatusLookup.PAID_USER);

                success = handlerDAO.getDbWrapper().addTransaction(swapMessage, desiredCurrencyValue);
                if (!success) {
